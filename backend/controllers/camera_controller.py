...
def frame_update_loop(manager):
    ...
    while manager.camera_active:
        if not manager.frame_queue.empty():
            try:
                frame = manager.frame_queue.get_nowait()
                manager.current_image = frame
                manager.frame_ready.emit(frame)
            except Empty:
                logger.debug("Frame queue empty, waiting for next frame")
        manager.thread.msleep(CAMERA_SLEEP_MS)
